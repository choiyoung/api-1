...
/*
성적표 백터로 작성하기 코딩 시나리오
1 . VO. Interface 클래스 다이어그램을 받아든다.  본인이 작성햇건 오더를 받았건,
일단 본인이 작성하지 않아서 잘모르는 상황이라는 설정.
혹은 본인이 하더라도 짐작가는대로 설정한 기능이라는 가정하예.
2.VO getter,setter ..implement 를 일단 구현한다.
3.보니까 기능 중에  TOtal 을 구하는 기능이 필요한데
필요한 것을 보니까 필드에 있는 과목점수가 있어야 겠기에
]그냥 VO 내부에 기능을 넣는다. 이정도 유연성은 개발자에게 있다.
4. 일단은 Vo 는 완성햇다고 보고 *impl.로 간다.
5. impl()을 보면서 .. 메소드를 분석해본다.
그리고 주석을 자신의 방식대로 적어둔다.
대충예를 들면
1-input() // 학생별로 성적을 입력 받는 것인가.. 그럼 입력받아서
저장할 공간인데 한번 생성하면 추가로 늘릴수 없으니
동적으로 확장하는 컬렉션을 선택ㅎ야겠군
그런데 내가 지금 아는 건 백터 뿐이니 백타에 넣어야지.
...
리턴값이 String 일테니 벡터의 for loop  +내부에 if 를 해봐야겠다.
	...
	리턴값이 하나 든 여러개든 for loop + if 구성빡에는
	백터요소 출력로직이 없으니 그걸 써야 겟다.
	5 - descGradeTotal()
	내림차순인가보다
