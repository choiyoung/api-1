...
배열에 더 이상 저장할 공간이없으면 보다 큰 새로운 배열을
...
ArrayList클래스
1.ArrayList 와 Vector class 는 list 를 크기 변경 가능함
배열로 구현한 것으로 , 차이점은 Vector 클래스의 ㄱ객체는
원래 기본적으로 동기화 되지만 ArrayList 는 그렇지 않다.

2. 비 동기화된 컬랙션은 동기화된 컬렉션보다 더 나은 성능을
제공하며 스레드들이 컬렉션을 공유하지 않는 프로그램에서는 
일반적으로 ArrayList  를 Vector 보다 더 선호 하며
ArrayList 는 동기화를 고려하지 않는 Vector 처럼
동작함으로써 스레드 동기화에 따르는 부담을 가지지 않기
때문에 Vector 보다 빠르게 실행된다.
